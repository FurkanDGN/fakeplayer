package tr.com.infumia.dantero.fakeplayer.nms.v1_8_R1;
...
final class Util {
    ...
    static void sendPacket(@NotNull final Player player, @NotNull final Packet... packets) {
        Arrays.stream(packets).forEach(((CraftPlayer) player).getHandle().playerConnection::sendPacket);
    }
    ...
    static void sendTabListAdd(@NotNull final EntityPlayer player) {
        Util.sendPacket(
            new PacketPlayOutPlayerInfo(EnumPlayerInfoAction.ADD_PLAYER, player)
        );
    }

    static void sendTabListRemove(@NotNull final EntityPlayer... players) {
        Util.sendPacket(
            new PacketPlayOutPlayerInfo(EnumPlayerInfoAction.REMOVE_PLAYER, players)
        );
    }
